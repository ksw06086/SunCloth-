...
var POPUP_LIFETIME_EVENT_GUIDE = {
    openPopup : function ()
    {
        if (POPUP_LIFETIME_EVENT_GUIDE.getCookie('LIFETIMEEVENT_LAYER_NEVER_OPEN') === 'T') {
            ...
            $.get('/member/lifetime_event.html','',function(sHtml)
            {
                if (bBuyLayer == true) {
                    if (parent.$('#lifetimeEventLayer').length <= 0) {
                        parent.$('body').append($('<div id=\"lifetimeEventLayer\"></div>'));
                        parent.$('#lifetimeEventLayer').html(sHtml);
                        parent.$('#lifetimeEventLayer').show();
                    }
                } else {
                    if ($('#lifetimeEventLayer').length <= 0) {
                        $('<div id=\"lifetimeEventLayer\"></div>').appendTo('body');
                        $('#lifetimeEventLayer').html(sHtml);
                        $('#lifetimeEventLayer').show();
                    }
                }
            });
        }
