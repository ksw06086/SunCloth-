						...
						<c:if test="${srhCnt > 0}">
							<c:forEach var = "list" items = "${list}">
								<tr style = "text-align:center;" class = "faq">
									...
									<!-- 상세 페이지 -->
									<td align = "left">
									<a href = "h_noticeForm?num=${list.num}&number=${number+1}&pageNum=${pageNum}&choose=${choose}">
									${list.subject}
									</a></td>
									<td>${list.id}</td>
										...
										<th align = "center">
											...
											<c:if test="${cnt > 0}">
												...
												<c:if test="${startPage > pageBlock}">
													<a href = "h_notice?choose=${choose}">[◀◀]</a>
													<a href = "h_notice?pageNum=${startPage - pageBlock}&choose=${choose}">[◀]</a>
												</c:if>
												...
												<c:forEach var = "i" begin = "${startPage}" end = "${endPage}">
													...
													<c:if test="${i != currentPage}">
														<span><a href = "h_notice?pageNum=${i}&choose=${choose}">[${i}]</a></span>				
													</c:if>
												</c:forEach>
												...
												<c:if test="${pageCount > endPage}">
													<a href = "h_notice?pageNum=${startPage + pageBlock}&choose=${choose}">[▶]</a>
													<a href = "h_notice?pageNum=${pageCount}&choose=${choose}">[▶▶]</a>
												</c:if>
											</c:if>
										</th>
									</tr>
